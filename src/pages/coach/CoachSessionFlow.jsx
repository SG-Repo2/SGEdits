...
const SESSION_STEPS = [
  { time: '5 min', icon: '💬', title: 'Check-In', desc: "Emotional + performance state. What's their energy? Any anxiety spikes? Did they complete the assigned work?" },
  { time: '20 min', icon: '🔬', title: 'Error Review', desc: 'Work through logged errors. Diagnose before correcting. Get reasoning first — classify category — then fix.' },
  { time: '25 min', icon: '🎯', title: 'Targeted Intervention', desc: 'Based on error category breakdown. Knowledge Gap → reteach. Classic Mix-Up → rebuild model. Brain Fart → timed drills.' },
  { time: '20 min', icon: '⚡', title: 'Performance Training', desc: 'Live questions under timed conditions. Coach decision-making in real time. Catch bad habits before test day.' },
  { time: '7 min', icon: '📣', title: 'Feedback Delivery', desc: 'One or two surgical observations. Observation → Mechanism → Correction. No praise without specificity.' },
  { time: '3 min', icon: '📋', title: 'Assignment + Tracking', desc: 'Exact tasks before next session. Update student portal. Log one session insight to Knowledge Engine.' },
];
...
const PROTOCOLS = {
  unsure: {
    ...
    steps: [
      { n: 1, action: 'Read stem fully — do not skip to answers', time: '0–15s' },
      { n: 2, action: 'Eliminate definitively wrong answers first', time: '15–30s' },
      { n: 3, action: "If 2 remain and you're under 90s: commit to best guess", time: '30–75s' },
      { n: 4, action: 'Flag for review — do NOT revisit until section end', time: '—' },
      { n: 5, action: 'Move immediately. Section momentum is non-negotiable', time: '—' },
    ]
  },
  reset: {
    ...
    steps: [
      ...
      { n: 5, action: 'Build momentum — treat first 3 items as warm-up', time: '—' },
    ]
  },
  morning: {
    title: 'Test Morning Routine',
    steps: [
      { n: 1, action: 'No studying morning of — zero content review for aggressive studiers. Light 15-min review for efficient studiers.', time: '7:00–7:15am' },
      { n: 2, action: 'Eat: complex carbs + protein. No caffeine spike.', time: '7:15am' },
      { n: 3, action: 'Arrive 20 min early. Environmental familiarity reduces anxiety.', time: '—' },
      { n: 4, action: 'One mental anchor phrase — written night before.', time: 'Pre-test' },
...
function FeedbackBuilder({ student }) {
  ...
  const TEMPLATES = [
    { obs: 'defaulting to keyword pattern-matching in Bio instead of reasoning through mechanisms', mech: "That's why you miss medium-difficulty questions — the keywords appear in wrong answers by design", corr: 'Verbalize the mechanism out loud before selecting. Every time. No exceptions.' },
    { obs: 'hesitating 15–20 seconds before committing to QR answers', mech: "Decision latency is consuming 30% of your QR time budget — you're running out of time, not knowledge", corr: "Set a 90-second hard cap. If you don't have the answer, mark your best guess and move. No revisits until the end." },
    { obs: 'confidence exceeding accuracy by approximately 8 points in Bio', mech: "You're pattern-matching to familiar concepts but not verifying — overconfidence is masking knowledge gaps", corr: 'Before submitting any Bio answer, state in one sentence why the other three choices are wrong.' },
  ];
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      <div className="panel">
        <div className="panel-header">
          ...
          <span className="tag tag-muted">Obs → Mech → Fix</span>
        </div>
        ...
        <button className="btn btn-gold" onClick={buildFeedback}>Build Feedback Statement →</button>
        ...
        {built && (
          <div style={{ marginTop: 14, padding: '16px 18px', borderRadius: 10, background: 'var(--gold-bg)', border: '1px solid var(--gold-border)' }}>
            ...
            <div style={{ marginTop: 10, display: 'flex', gap: 8 }}>
              <span className="tag tag-success">✓ Specific</span>
              <span className="tag tag-success">✓ Behavioral</span>
              <span className="tag tag-success">✓ Actionable</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="panel">
        ...
        <div style={{ marginTop: 14, padding: '12px 14px', borderRadius: 10, background: 'var(--danger-bg)', border: '1px solid var(--danger-border)' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--danger)', marginBottom: 6, textTransform: 'uppercase' }}>Vague Feedback — Auto-Flag</div>
          <div style={{ fontSize: 12, color: 'var(--text-lo)', lineHeight: 1.8 }}>
            ✗ "You need to study Bio more"<br />
            ✗ "Try to be more careful"<br />
            ✗ "Good job on that section"
          </div>
        </div>
      </div>
    </div>
...
function EnrollmentFilter() {
  ...
  const evaluate = () => {
    ...
    if (gpa < 3.0) { issues.push('GPA below 3.0 — DAT score alone cannot overcome this gap. Recommend GPA strategy first.'); viable = false; }
    if (weeks < 4) { issues.push('Less than 4 weeks — insufficient time for meaningful score improvement. Recommend rescheduling test date.'); viable = false; }
    if (weeks < 6 && target >= 23) { issues.push('6 weeks is not enough to reach 23+. Set realistic expectation or recommend 10-week program.'); }
    if (target > 25) { issues.push('25+ is elite territory. Flag as aspirational — set primary target at 23.'); }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      <div className="panel">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
          ...
          <div><label className="form-label">Target Score</label><input className="form-input" value={form.target} onChange={e => setForm({ ...form, target: e.target.value })} placeholder="23" type="number" /></div>
        </div>
        <button className="btn btn-gold" onClick={evaluate}>Evaluate Fit →</button>
        ...
        {result && (
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 800, color: result.viable ? 'var(--success)' : 'var(--danger)', marginBottom: 10 }}>
              {result.viable ? '✓ VIABLE ENROLLMENT' : '✗ NOT RECOMMENDED AT THIS TIME'}
            </div>
      ...
      <div className="panel">
        ...
        {[
          ...
          { rule: "Offer an alternative, not just a no", detail: "If you can't help them now, tell them what they need to do first. They'll come back — and refer others." },
            ...
            {SESSION_STEPS.map((s, i) => (
              <div key={i} onClick={() => setStep(step === i ? -1 : i)} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <span style={{ color: 'var(--text-lo)', fontSize: 12 }}>{step === i ? '▲' : '▼'}</span>
                </div>
                {step === i && (
        ...
        <div className="panel">
          ...
          <div style={{ background: 'var(--bg-panel-hover)', borderRadius: 12, padding: 16, border: '1px solid var(--border)' }}>
            ...
            {PROTOCOLS[protocol].steps.map(s => (
              <div key={s.n} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 10 }}>
                ...
                {s.time !== '—' && <div style={{ fontSize: 11, color: 'var(--text-lo)', flexShrink: 0, fontFamily: 'monospace' }}>{s.time}</div>}
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 20 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-hi)', marginBottom: 12 }}>
              2-Week Peak Protocol — {student.name}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { day: 'Day 14–10', load: 'Heavy', focus: 'Full practice tests + deep error review' },
                { day: 'Day 9–5', load: 'Moderate', focus: 'Targeted section drilling on weak areas' },
                { day: 'Day 4–2', load: 'Light', focus: 'Short drills, confidence areas only' },
